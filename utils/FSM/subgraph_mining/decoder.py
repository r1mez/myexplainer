...
from utils.FSM.common.combined_syn import get_dataset
...
def pattern_growth(dataset, task, args, label=None):
    # init model
    ...
    for i, graph in enumerate(dataset):
        ...
        if not type(graph) == nx.Graph:
            if args.dataset == "mutag":
                graph_nx = pyg_utils.to_networkx(graph, node_attrs=['x'], edge_attrs=['edge_attr']).to_undirected()
                # 新增：转换所有边属性
        ...
        if args.sample_method == "radial":
            for i, graph in enumerate(graphs):
                ...
                for j, node in enumerate(graph.nodes):
                    ...
                    if len(neigh) > 1:
                        # ✅ 修复：使用.copy()创建真实副本（而非view），确保属性被保留
                        neigh = graph.subgraph(neigh).copy()
                        if args.subgraph_sample_size != 0:
                            neigh = neigh.subgraph(max(
                                nx.connected_components(neigh), key=len)).copy()
                        # 转换节点标签，NetworkX会自动保留节点和边属性
    ...
    for pattern in out_graphs:
        ...
        os.makedirs("plots/{}/{}".format(args.dataset, label), exist_ok=True)
        print(f"Saving plots/{args.dataset}/{label}/{len(pattern)}-{count_by_size[len(pattern)]}.png")
        plt.savefig(f"plots/{args.dataset}/{label}/{len(pattern)}-{count_by_size[len(pattern)]}.png")
        # plt.savefig(f"plots/{args.dataset}/{label}/{len(pattern)}-{count_by_size[len(pattern)]}.pdf")
        plt.close()
        count_by_size[len(pattern)] += 1

    if label is None:
        args.out_path = f"fsm_results/{args.dataset}_patterns.pkl"
    else:
        args.out_path = f"fsm_results/{args.dataset}_{label}_patterns.pkl"
...
def FSMiner(dataset, label):
    ...
    task = 'graph'
    pattern_growth(dataset, task, args, label)
...
def main():
    ...
    if args.dataset == 'enzymes':
        ...
        dataset, _, _ = get_datasets(name='mutag')
        task = 'graph'
    elif args.dataset == 'ba2motif':
        dataset, _, _ = get_datasets(name='ba2motif')
        task = 'graph'
