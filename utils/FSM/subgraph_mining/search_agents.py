...
class MCTSSearchAgent(SearchAgent):
    def __init__(self, min_pattern_size, max_pattern_size, model, dataset,
        ...
        """ MCTS implementation of the subgraph pattern search.zz
    ...
    def finish_search(self):
        ...
        for pattern_size in range(self.max_pattern_size, self.min_pattern_size-1, -1):
            # 新增：为每个高频组计算密度（优先高密度，然后高 counts）
            scored_groups = []
            for wl_hash, count in counts[pattern_size].items():
                neighs = self.wl_hash_to_graphs[wl_hash]
                if not neighs:  # 空组跳过
                    continue
                # 选密度最高的作为代表
                best_neigh = max(neighs, key=lambda g: nx.density(g))  # 选密度最高的
                density = nx.density(best_neigh)  # 计算密度
                # 调整排序键顺序——密度主要（降序），counts 次要（降序）
                scored_groups.append((-density, -count, best_neigh))  # 多键：density 降序优先，counts 降序次之

            # 修改：按多键排序，取前 out_batch_size
            for _, _, pattern in sorted(scored_groups, key=lambda x: (x[0], x[1]))[:self.out_batch_size]:
                cand_patterns_uniq.append(pattern)
                print("- outputting motif of size", pattern_size, "with density", nx.density(pattern), "and count",
                      counts[pattern_size][utils.wl_hash(pattern, node_anchored=self.node_anchored)])

        for pattern in cand_patterns_uniq:
    ...
    def finish_search(self):
        ...
        for pattern_size in range(self.max_pattern_size, self.min_pattern_size-1, -1):
            ...
            if cur_rank_method == "margin":
                ...
                # 新增：为每个高频组计算 cc（随机选时优先高密度图）
                scored_groups = []
                for wl_hash, neighs in self.counts[pattern_size].items():
                    # 选密度最高的作为代表（从 diameter 改为 density）
                    best_neigh = max(neighs, key=lambda g: nx.density(g))  # 选密度最高的
                    density = nx.density(best_neigh)  # 计算密度
                    count = len(neighs)
                    # 修改：调整排序键顺序——密度主要（降序），counts 次要（降序）
                    scored_groups.append((-density, -count, best_neigh))  # 多键：density 降序优先，counts 降序次之
