    ...
    if task == "graph":
        ...
        for i, graph in tqdm(enumerate(dataset)):
            if not type(graph) == nx.Graph:
                ...
                if name == 'mutag':
                    graph = pyg_utils.to_networkx(graph, node_attrs=['x'], edge_attrs=['edge_attr']).to_undirected()
                else:
                    graph = pyg_utils.to_networkx(graph, node_attrs=['x']).to_undirected()
