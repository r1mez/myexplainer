...
from utils.graph_utils import data_to_mol, smarts_to_data, smarts_to_mol


def find_largest_subgraph(dataset_name, smiles_list, data, invalid_smis):
    """
    在 SMILES 列表中找到最大的子图，该子图是 Data 分子图的子图
    返回最大的子图的 SMILES 字符串
    """
    # 过滤掉无效的 SMILES
    smiles_list = [s for s in smiles_list if s not in invalid_smis]
    # 将 Data 转换为 RDKit 分子
    try:
        target_mol = data_to_mol(dataset_name, data)[0]
        target_mol = Chem.RemoveHs(target_mol)  # 移除氢原子以简化匹配
    except Exception as e:
        print(f"Error converting Data to molecule: {e}")
        return None

    max_subgraph_smiles = None
    max_atom_count = -1
    ...
    for smiles in smiles_list:
        try:
            ...
            if sub_mol is None:
                continue
            sub_mol = Chem.RemoveHs(sub_mol)  # 移除氢原子

            # 检查是否是子图
            if target_mol.HasSubstructMatch(sub_mol):
                # 计算原子数
                atom_count = sub_mol.GetNumAtoms()
                if atom_count > max_atom_count:
                    max_atom_count = atom_count
                    max_subgraph_smiles = smiles
        except:
            continue  # 跳过无效的 SMILES
    subgraph = smarts_to_data(dataset_name, max_subgraph_smiles)
    # return max_subgraph_smiles
    return subgraph, max_subgraph_smiles
...
# ):
#     """
#     根据给定的 SMILES 和 Data 对象，生成子图的节点和边掩码
#
#     参数：
