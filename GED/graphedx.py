import torch
from .ged_utils import gmngen

from .ged_utils.model_utils import get_graph_features, split_and_stack, pytorch_sinkhorn_iters, asymm_embed_mat_l1_dist, compute_loss
...
class GRAPHEDX_xor_on_edge(torch.nn.Module):
    def __init__(self, conf, gmn_config):
        ...
        self.source_destination_list = torch.ones(self.max_node_set_size, self.max_node_set_size,
                                                  device=self.device).triu(1).nonzero()
        self.source_list = self.source_destination_list[:, 0]
        self.destination_list = self.source_destination_list[:, 1]

        self.edge_source_dest_idx = torch.ones(self.node_set_size_nC2, self.node_set_size_nC2,
        ...
        if conf.data_mode == "unequal":
            ...
            print(
                f"Costs: {self.node_ins_cost}, {self.node_del_cost}, {self.node_rel_cost}, {self.edge_ins_cost}, {self.edge_del_cost}, {self.edge_rel_cost}")
    ...
    def forward(self, graphs, graph_sizes, query_adj, corpus_adj):
        ...
        stacked_query_node_features, stacked_corpus_node_features = split_and_stack(
        ...
        if self.use_max:
            edge_sinkhorn_input = torch.maximum(straight_score, cross_score).reshape(-1, self.node_set_size_nC2,
                                                                                     self.node_set_size_nC2)
        else:
            edge_sinkhorn_input = (straight_score + cross_score).reshape(-1, self.node_set_size_nC2,
                                                                         self.node_set_size_nC2)

        if self.use_second_sinkhorn:

            edge_transport_plan = pytorch_sinkhorn_iters(
                log_alpha=edge_sinkhorn_input, device=self.device, temperature=self.sinkhorn_temp,
                noise_factor=self.conf.training.sinkhorn_noise
            )
        elif self.use_second_sinkhorn_log:
            edge_transport_plan = pytorch_sinkhorn_iters(
                log_alpha=torch.log(edge_sinkhorn_input + 1e-6), device=self.device, temperature=self.sinkhorn_temp,
                noise_factor=self.conf.training.sinkhorn_noise
            )
        else:
        ...
        if self.use_m_ms_edge:
            ...
            edge_align_dist = (pairwise_xor * edge_transport_plan * (
                        self.edge_del_cost * torch.nn.functional.relu(edge_emb_pairwise_diff).sum(dim=-1) \
                        + self.edge_ins_cost * torch.nn.functional.relu(-edge_emb_pairwise_diff).sum(dim=-1))).sum(
                dim=(-1, -2))
        else:
            edge_align_dist = asymm_embed_mat_l1_dist(alpha_query[:, :, None] * nC2_query_edge_embeddings,
                                                                  alpha_corpus[:, :, None] * nC2_corpus_edge_embeddings,
                                                                  edge_transport_plan, self.edge_ins_cost,
                                                                  self.edge_del_cost)

        if self.use_h_hp_node:
            node_emb_pairwise_diff = stacked_query_node_features[:, :, None, :] - stacked_corpus_node_features[:, None,
                                                                                  :, :]
            node_align_dist = (node_transport_plan * (
                        self.node_del_cost * torch.nn.functional.relu(node_emb_pairwise_diff).sum(dim=-1) \
                        + self.node_ins_cost * torch.nn.functional.relu(-node_emb_pairwise_diff).sum(dim=-1))).sum(
                dim=(-1, -2))
        else:
            node_align_dist = asymm_embed_mat_l1_dist(stacked_query_node_features,
                                                                  stacked_corpus_node_features, node_transport_plan,
                                                                  self.node_ins_cost, self.node_del_cost)

        return edge_align_dist + self.LAMBDA * node_align_dist

    def compute_loss(self, lower_bound, upper_bound, out):
        return compute_loss(lower_bound, upper_bound, out)